...
点开项目结构——>模块——>选中需要的模块——>点击+号添加Web——>在点击Web中的+号添加一个web.xml
...
servlet.xml，例如，以下配置所对应SpringMVC的配置文件位于WEB-INF下

文件名为springMVC-servlet.xml
...
## 5.7、向session域共享数据
 ...
 session域的数据是存储在服务器端的内存中的，每个用户都有一个对应的session对象。服务器会为每个新的会话创建一个新的session对象，而当会话结束时，该session对象将被销毁。 

 session域适用于存储一些与用户相关的信息，如用户登录状态、购物车内容等。因为它只在会话期间内有效，所以可以有效地避免多个用户之间的数据混淆。 
...
## 5.8、向application域共享数据
 ...
 application域的数据是存储在服务器的内存中的，只有一个application对象。当Web应用程序启动时，服务器会创建一个新的application对象，当Web应用程序关闭时，该application对象将被销毁。 

 application域适用于存储一些全局性的信息，如全局配置参数、共享数据等。因为它在整个应用程序的生命周期内都有效，所以可以为多个用户共享数据提供方便。但是，由于数据是全局共享的，因此需要特别注意多线程并发访问的安全问题。 
...
tomcat的web. xm配置的Defaul tServlet的url -pattern也是/

当前工程的web.xml最后会继承tomcat的web.xml
...
若配置了<mvc:default-servlet-handler />和<mvc : annotation-drivqn />
浏览器发送的请求会先被Dispatcherservlet处理，无法处理在交给DefaultServLet 处理

```xml
<mvc:default-servlet-handler />

<!--开启mvc注解驱动-->
<mvc:annotation-drivqn />
```
...
>
> 2、{key:value,key:value,...}，(json)此时无法通过request.getParameter()获取，之前我们使用操作
>
...
preHandle()会按照配置的顺序执行，而postHandle()和afterCompletion()会按    照配置的反序执行
