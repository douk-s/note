...
> 属性：
>
> default：设置默认使用的环境id（environment的id）
>
> 
>
> environment设置一个具体的连接数据库的环境
>
> 属性：
>
> id：设置环境的唯一标识，不能重复
...
在配置xml文件中引入properties配置文件

1写好jdbc.properties文件

2在xml中加上<properties resource="jdbc.properties" />标签
...
> 		属性：
> 			type：设置需要设置别名的类型
> 			alias：设置某个类型的别名，若不设置该属性，那么该类型拥有默认的别名——类名，即类名且不区分大小写
> 	-->
...
> 		也就是在resources下的文件夹和接口的包名对应
> 		创建包时用.分隔，在resources时用/
> 	-->
...
> mepper:
>
> select(@Param("username") String username,@Param("password") String password);
>
> 以param1,param2...为键，以参数为值；只需要通过${}和#{}访问map集合的键就可以获取相对应
>
> 的值，
>
> 注意${}需要手动加单引号
...
# 8、自定义映射resultMap（处理属性名和表字段名不一样的方法）

## 8.1、resultMap处理字段和属性的映射关系

若字段名和实体类中的属性名不一致可以在sql语句中用as起一个别名与属性名一致
...
<resultMap id="empDeptStepMap" type="Emp">
	...
	-->
	<association property="dept"select="com.atguigu.MyBatis.mapper.DeptMapper.getEmpDeptByStep" column="d_id">
</association>
</resultMap>
<!--Emp getEmpByStep(@Param("eid") int eid);-->
...
<!--        按需加载-->
        <setting name="aggressiveLazyLoading" value="false"/>s
    </settings>
```
...
<generatorConfiguration>
    ...
    <context id="DB2Tables" targetRuntime="MyBatis3">
        ...
        <jdbcConnection driverClass="com.mysql.cj.jdbc.Driver"
                        connectionURL="jdbc:mysql://localhost:3306/db_mybatis?serverTimezone=UTC"
                        userId="root"
                        password="1234">
        </jdbcConnection>
        <!-- javaBean的生成策略-->
        <javaModelGenerator targetPackage="com.douk.mybatis.pojo" targetProject=".\src\main\java">
<!--            是否使用子包-->
...
<!--            根据表字段生成-->
        ...
        <!-- SQL映射文件的生成策略 -->
        <sqlMapGenerator targetPackage="com.douk.mybatis.mapper" targetProject=".\src\main\resources">
            <property name="enableSubPackages" value="true" />
        </sqlMapGenerator>
        <!-- Mapper接口的生成策略 -->
        <javaClientGenerator type="XMLMAPPER" targetPackage="com.douk.mybatis.mapper" targetProject=".\src\main\java">
            <property name="enableSubPackages" value="true" />
        </javaClientGenerator>
        <!-- 逆向分析的表 -->
...
a>在查询功能之前java代码中使用PageHelper.startPage(int pageNum, int pageSize)开启分页功能
...
> list：分页之后的数据
>
> navigatePages：导航分页的页码数写5就是 3 4 5 6 7
