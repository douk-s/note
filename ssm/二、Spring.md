...
DI 是 IOC 的另一种表述方式：即组件以一些预先定义好的方式（例如：set 方法）接受来自于容器
...
public void testLife(){
    //ConfigurableApplicationContext是ApplicationContext的于接口，其中扩展了刷新和关闭容器的方法
...
在成员变量或者变量的set方法或者有参构造上直接标记@Autowired注解即可完成自动装配，不需要提供setXxx()方法。以后我们在项
...
### 3.3.1、概述
 ...
 AOP（面向切面编程）是一种编程范式，它将应用程序的功能分为核心功能和横切关注点。核心功能是应用程序的主要业务逻辑，而横切关注点则是指与核心功能相关的但与应用程序的实际业务逻辑无关的功能，例如日志记录、性能监控和安全性检查等。 

 AOP 的目的是将横切关注点从核心功能中分离出来，以提高代码的可重用性、可维护性和可扩展性。它通过使用称为“切面”的组件来实现这一目的。切面是一组与横切关注点相关的行为，它们可以在运行时被动态地织入到应用程序的核心功能中。 
...
- 后置通知：使用@After注解标识，在被代理的目标方法**最终结束**后执行finally之后（**盖棺定论**）
...
@RunWith(SpringJUnit4ClassRunner.class)导入springxml的bean
